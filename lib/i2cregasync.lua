...
function REG:r(reg, num, cb)
    num = num or 1
    local arr = storm.array.create(1, storm.array.UINT8)
    arr:set(1, reg)
    storm.i2c.write(self.port + self.address, storm.i2c.START, arr, function (rv)
        if (rv ~= storm.i2c.OK) then
            print ("ERROR ON I2C: ",rv)
            cb(nil)
        end
        local result = storm.array.create(num, storm.array.UINT8)
        storm.i2c.read(self.port + self.address, storm.i2c.RSTART + storm.i2c.STOP, result, function (rv)
            if (rv ~= storm.i2c.OK) then
                print ("ERROR ON I2C: ",rv)
                cb(nil)
            end
...
function REG:w(reg, values, cb)
    ...
    storm.i2c.write(self.port + self.address, storm.i2c.START + storm.i2c.STOP, arr, function (rv)
        if (rv ~= storm.i2c.OK) then
            print ("ERROR ON I2C: ",rv)
        end
        storm.os.invokeLater(storm.os.MILLISECOND, cb, 1)
    end)
end
